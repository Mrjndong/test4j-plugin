...
import org.jtester.plugin.resources.ResourceHelper;
import org.jtester.savexp.xstream.ExXStream;

public class SaveExpHelper {
	private final static SimpleDateFormat dateFormat = new SimpleDateFormat("yyMMdd HHmmss.SSS");

	private final static String SAVE_EXPRESSION_FILE_SUFIX = ".xml";
	 ...
	 * @return
	 */
	public static String getXmlFile(final String basePath, final String variableName) {
	 ...
	 *            存储文件的路径名称
	 */
	public static void toXML(IValue o, String filename) {
		try {
			ExXStream xs = new ExXStream();
			FileOutputStream fos = new FileOutputStream(filename);
			xs.toXML(o, fos);
			fos.close();
		} catch (Throwable e) {
			e.printStackTrace();
			PluginLogger.log("save expression to xml excpetion", e);
