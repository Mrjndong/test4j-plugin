...
public class Test4JPreferenceInitializer extends AbstractPreferenceInitializer {
    private IPreferenceStore store;

    public Test4JPreferenceInitializer() {
        this.store = Test4JActivator.preference();
    }

    public void initializeDefaultPreferences() {
        store.setDefault(PluginSetting.DATABASE_URL, "jdbc:mysql://127.0.0.1/test4j?characterEncoding=UTF8");
        ...
        try {
            store.setDefault(PluginSetting.DOMAIN_SQLMAP_TEMPLATE,
                    VelocityUtil.readTemplate(CommonConstants.DOMAIN_SQLMAP_TEMPLATE));
            store.setDefault(PluginSetting.DOMAIN_ENTITY_TEMPLATE,
                    VelocityUtil.readTemplate(CommonConstants.DOMAIN_ENTITY_TEMPLATE));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
